...
docker build -t cheqmate-engine .
docker ps
docker logs

docker run -d --name cheqmate-engine --restart unless-stopped -p 8000:8000 -v C:\moodle\server\moodledata:/moodledata cheqmate-engine



Dont❌ docker run -d --name cheqmate-engine -p 8000:8000 -v C:\moodle\server\moodledata:/moodledata cheqmate-engine
